...
def simulate_inventory(forecast_df, reorder_point, lead_time, initial_inventory, days, num_simulations=500):
    ...
    stds = ((forecast_df['yhat_upper'] - forecast_df['yhat_lower']
             ) / 4).clip(lower=0).values[-days:]
...
def show_forecast():
    ...
    st.markdown(
        "- Requires columns: **Product_Name**, **Season**, **Sales_Last_30_Days**, optional **Profit_Per_Unit**.")
    ...
    product = st.selectbox("Which product to forecast?",
    ...
    volatility = st.slider(
        "Synthetic history volatility (%)", 0, 100, 10) / 100.0

    # STEP 5: Generate synthetic 30-day history
    st.header("5️⃣ Generate synthetic 30-day history")
    st.markdown(
        "• A synthetic series is generated using your average daily sales and randomized noise.")
    st.markdown(
        "• This imitates the past 30 days of sales data, providing context for the forecast.")
    st.markdown(
        "• It visually demonstrates recent historical trends to inform the forecasting model.")
    ...
    fig_hist.update_layout(title='Last 30 Days Synthetic History',
    ...
    st.markdown(
        "• The Prophet model is trained on the synthetic history to capture trends and seasonality.")
    st.markdown(
        "• A forecast is generated for the next 15 days based on the historical patterns.")
    st.markdown(
        "• This step predicts future sales and highlights expected trends over the short term.")
    ...
    fig_fc.add_trace(go.Scatter(
        x=ts['ds'], y=ts['y'], mode='markers', name='History'))
    fig_fc.add_trace(go.Scatter(x=forecast['ds'].tail(
        horizon), y=forecast['yhat'].tail(horizon), mode='lines', name='Forecast'))
    ...
    st.markdown(
        "• This component plot breaks down the forecast into trend, seasonal, and holiday effects.")
    st.markdown(
        "• It helps you understand the underlying drivers of the forecasted sales patterns.")
    st.markdown(
        "• Use these insights to adjust your business strategy accordingly.")
    ...
    if 'Profit_Per_Unit' in subset.columns:
        ...
        fig_rev = go.Figure(go.Bar(x=forecast['ds'].tail(
            horizon), y=forecast['revenue'].tail(horizon)))
        fig_rev.update_layout(title='Revenue Forecast',
    ...
    with st.expander("🔄 Interactive Inventory Simulation"):
        st.markdown(
            "**Configure simulation parameters and explore inventory dynamics.**")
        st.markdown(
            """
...
- **Sample Inventory Trajectories:** Day-by-day inventory trends for example scenarios."""
        ...
        init_inv = st.number_input(
            "Initial Inventory (units)", value=int(reorder_pt * 2))
        sims = st.slider("Number of Simulations", 10, 1000, 200, step=10)
        samples = st.slider("Sample Trajectories to Display", 1, 10, 5)

        stats_df = simulate_inventory(
            forecast, reorder_pt, lead_time, init_inv, horizon, sims)
        st.metric("Avg. Stockouts per Sim",
                  f"{stats_df['stockouts'].mean():.1f}")
        st.metric("Avg. Ending Inventory",
                  f"{stats_df['avg_inventory'].mean():.0f}")

        col5, col6 = st.columns(2)
        with col5:
            fig1 = go.Figure(go.Histogram(x=stats_df['stockouts']))
            fig1.update_layout(title='Stockouts Distribution',
                               xaxis_title='Stockouts', yaxis_title='Count')
            st.plotly_chart(fig1, use_container_width=True)
        with col6:
            fig2 = go.Figure(go.Histogram(x=stats_df['avg_inventory']))
            fig2.update_layout(title='Avg Inventory Distribution',
                               xaxis_title='Avg Inventory', yaxis_title='Count')
        ...
        for sid in sample_ids:
            ...
            fig3.add_trace(go.Scatter(
                x=sim_data['day'], y=sim_data['inventory'], mode='lines', name=f"Sim {sid}"))
        fig3.update_layout(title='Sample Inventory Trajectories',
